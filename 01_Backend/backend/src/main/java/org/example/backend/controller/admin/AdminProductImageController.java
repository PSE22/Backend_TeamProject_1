...
import org.example.backend.service.admin.AdminProductImageService;
...
public class AdminProductImageController {
    ...
    public ResponseEntity<Object> findAll(
            @RequestParam(defaultValue = "") Long pdImgId,
...
//               아래 코드는 프론트로(웹브라우저) 신호를(500) 보냄
    ...
    //    TODO: 상세조회
    @GetMapping("/admin-product-image/{pdImgId}")
    public ResponseEntity<Object> findById(
            @PathVariable long pdImgId
    ) {
        try {
//            DB 상세조회 서비스 함수 실행
            Optional<ProductImage> optionalProductImage = adminProductImageService.findById(pdImgId);
...
//                데이터 있음(200)
                return new ResponseEntity<>(optionalProductImage.get()
                        , HttpStatus.OK);
            }
...
//            500 전송
    ...
    //    TODO: 수정함수
    @PutMapping("/admin-product-image/{pdImgId}")
...
//            DB 에러 (서버 에러) -> 500 신호(INTERNAL_SERVER_ERROR)
    ...
    //     TODO: 삭제 함수
    @DeleteMapping("/admin-product-image/deletion/{pdImgId}")
    public ResponseEntity<Object> delete(
            @PathVariable long pdImgId
    ) {
        try {
//            DB 서비스 삭제 함수 실행
            boolean success = adminProductImageService.removeById(pdImgId);
