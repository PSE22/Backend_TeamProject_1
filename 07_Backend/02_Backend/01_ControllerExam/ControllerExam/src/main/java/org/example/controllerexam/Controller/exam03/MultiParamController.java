...
//    TODO: 예제 1) URL 매개변수 2개를 각각 jsp 로 전달하세요
//       URL 테스트 : http:m0//localhost:8000/exam3/hello-name-id?name=honggildong&id=hong
//       함수명 : setHelloNameId
    @GetMapping("/hello-name-id")
    public String setHelloNameId(Model model ,@RequestParam(defaultValue = "") String name,
...
//    url 테스트 : http://localhost:8000/exam03/example01?dno=10&dname=Accounting&loc=NewYork
    ...
    public String setExample01(Model model ,@RequestParam(defaultValue = "0") int dno,
...
//    함수 : setHelloArray
//    url : /hello-array
//    jsp : exam03/hello_array.js
    @GetMapping("/hello-array")
    public String setHelloArray(Model model ,@RequestParam(defaultValue = "0") String name,
                                            @RequestParam(defaultValue = "") String id)
    {
//      향상된 배열 : Arraylist
...
//          함수 : 변수명.add(값) , 변수명.get(인덱스번호)
        ...
        model.addAttribute("list", list); // model 로 보내서 jsp 에서 모델의 키(list) 로 받음
...
//      복습 : List<자료형> 변수명 = new ArrayList<>();
//        함수 : 변수명.add(값) , 변수명.get(인덱스번호)
//        List<String> <- list 만들때 자료형을 String만 들어올수 있게 해놨기 때문에
//         String.valueOf(변수명) 을 해서 정수를 문자열로 바꿔줘야함
        List<String> list = new ArrayList<>();
