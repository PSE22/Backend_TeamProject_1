...
<%-- 부트스트랩 복습 : container 클래스 : 가상의 페이지 중앙정렬 --%>
    <div class="container">
        <h2>산술연산자 / 테스팅</h2>
<%-- 부트스트랩 복습 : ms-3(margin start, 왼쪽마진), mt-3(margin top, 위쪽마진) --%>
        <div class="ms-3 mt-3">
            <p>문자열 : ${"문자열 출력됩니까? O : 출력성공" }</p>
<%--            <p>문자열 더하기 : ${"문자열1" + "문자열2" X : 출력실패 -> 그냥 아래처럼 문자열 2개쓰기 }</p>--%>
            <p>${"문자열1"} ${"문자열2"}</p>
<%--            jsp 에서는 더하기는 산술연산만 가능 문자열 안됨 --%>
<%--            strNum 은 "10"으로 문자열로 줬지만 자동으로 숫자로 변환됨 --%>
            <p> 덧셈("10") : ${strNum + 10}</p>
<%--            사칙연산 테스트 : --%>
            <p> 덧셈(10) : ${iNum + 10} </p>
            <p> 뺄셈(10) : ${iNum - 10} </p>
...
<%--            3항연산자(조건문) : (조건식)? "실행1": "실행2"--%>
            <p> 조건(3항연산자) : ${(iNum == 100)? "100" : "100 이 아님"}</p>
