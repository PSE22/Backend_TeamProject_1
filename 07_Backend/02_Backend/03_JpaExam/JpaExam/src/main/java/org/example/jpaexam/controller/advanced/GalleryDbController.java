package org.example.jpaexam.controller.advanced;

import lombok.extern.slf4j.Slf4j;
import org.example.jpaexam.model.entity.advanced.FileDb;
import org.example.jpaexam.model.entity.advanced.GalleryDb;
import org.example.jpaexam.service.advanced.FileDbService;
import org.example.jpaexam.service.advanced.GalleryDbService;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.data.domain.Page;
import org.springframework.data.domain.PageRequest;
import org.springframework.data.domain.Pageable;
import org.springframework.http.HttpHeaders;
import org.springframework.http.ResponseEntity;
import org.springframework.stereotype.Controller;
import org.springframework.ui.Model;
import org.springframework.web.bind.annotation.*;
import org.springframework.web.multipart.MultipartFile;
import org.springframework.web.servlet.view.RedirectView;

import java.util.Optional;

/**
 * packageName : org.example.jpaexam.controller.advanced
 * fileName : GalleryDbController
 * author : GGG
 * date : 2024-03-25
 * description :
 * 요약 :
 * <p>
 * ===========================================================
 * DATE            AUTHOR             NOTE
 * -----------------------------------------------------------
 * 2024-03-25         GGG          최초 생성
 */
@Slf4j
@Controller
@RequestMapping("/advanced")
public class GalleryDbController {
    @Autowired
    GalleryDbService galleryDbService;  // DI

    //  전체 조회 + like 검색 + 페이징
    @GetMapping("/galleryDb")
    public String getGalleryDbAll(
            @RequestParam(defaultValue = "") String galleryTitle,
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "3") int size,
            Model model) {
        Pageable pageable = PageRequest.of(page, size);
//        DB like 검색 서비스 함수 실행 : 페이징 처리
        Page<GalleryDb> pageRes
                = galleryDbService
                .findAllByGalleryTitleContaining(galleryTitle, pageable);

        model.addAttribute("galleryDb", pageRes.getContent());            // 1) fileDb 정보(배열)
        model.addAttribute("currentPage", pageRes.getNumber());        // 2) 현재페이지번호
        model.addAttribute("totalItems", pageRes.getTotalElements());  // 3) 전체 테이블 건수
        model.addAttribute("totalPages", pageRes.getTotalPages());     // 4) 전체 페이지 개수

//     공식 : 블럭 시작페이지 번호 = (Math.floor(현재페이지번호/1페이지당개수)) * 1페이지당개수
//     TODO: size -> 3
//       페이지번호 화면에 보이기 변수
        long pageNo = 4;
        long blockStartPage = (long) Math.floor((double) (pageRes.getNumber()) / pageNo) * pageNo;
        model.addAttribute("startPage", blockStartPage);              // 5) 블럭 시작페이지번호
//     공식 : 블럭 끝페이지 번호 = 블럭 시작페이지번호 + 1페이자당개수 - 1
//       TODO: size -> 3
        long blockEndPage = blockStartPage + pageNo - 1;
//        블럭 끝페이지 번호 >=  전체페이지번호 : 이 경우가 발생할 수 있음
//        블럭 끝페이지 번호 = 전체페이지번호 - 1 (값 보정)
        blockEndPage = (blockEndPage >= pageRes.getTotalPages()) ? pageRes.getTotalPages() - 1 : blockEndPage;
        //        TODO: blockEndPage < 0 이면 blockEndPage = 0 고정 : blockEndPage 음수이면 jsp 반복문에서 에러발생(버그 수정)
        blockEndPage = (blockEndPage < 0) ? 0 : blockEndPage;

        model.addAttribute("endPage", blockEndPage);
//        TODO: fileTitle -> jsp 전송
        model.addAttribute("galleryTitle", galleryTitle);

        return "advanced/galleryDb/galleryDb_all.jsp";
    }

    //    저장 : 1) 저장(추가) 페이지 열기 함수
    @GetMapping("/galleryDb/addition")
    public String addGalleryDb(){
        return "advanced/galleryDb/add_galleryDb.jsp";
    }

    //   2) 저장 버튼클릭시(파일업로드 실행될 함수)
//    insert -> post 방식 -> @PostMapping
//    파일업로드 : 쿼리스트링 : @RequestParam
    @PostMapping("/galleryDb/add")
    public RedirectView createGalleryDb(
            @RequestParam(defaultValue = "") String galleryTitle,
            @RequestParam MultipartFile image // 파일 전송 : MultipartFile
    ){
        try {
//            DB 저장 서비스 함수 실행
            galleryDbService.save(null,galleryTitle,image);
        } catch (Exception e){
            log.debug(e.getMessage());
        }
        return new RedirectView("/advanced/galleryDb");
    }

//    3) 파일 다운로드 함수
    @GetMapping("/galleryDb/{uuid}")
    @ResponseBody   // json 데이터로 전송하게하는 어노테이션
    public ResponseEntity<byte[]> findDownload(@PathVariable String uuid) {
//       DB 상세 조회 서비스 함수 실행 : uuid
        GalleryDb galleryDb = galleryDbService.findById(uuid).get();
//        jsp 전송 : json 형태로 전송
        return  ResponseEntity.ok()
//          파일 전송  :
//                    header : 파일의형태, 파일명을 알려줌
//                       1) 파일형태 : CONTENT_DISPOSITION
//                       2) 첨부파일 : attachment; filename="xxx.jpg"
//                    body : 실제 이미지파일 데이터
                .header(HttpHeaders.CONTENT_DISPOSITION
                        ,"attachment; filename=\"" + galleryDb.getGalleryFileName() + "\"")
                .body(galleryDb.getGalleryData()); // 실제 이미지 전송
    }

    //    삭제 함수 : 기본키(uuid)
//    delete -> delete 방식 -> @DeleteMapping
    @DeleteMapping("/galleryDb/delete/{uuid}")
    public RedirectView deleteGalleryDb(
            @PathVariable String uuid
    ) {
//        DB 삭제 서비스 실행
        galleryDbService.removeById(uuid);
//        jsp 전체 조회 페이지 강제 이동
        return new RedirectView("/advanced/galleryDb");
    }

    //    수정 : 1) 수정 페이지 열기 : 상세조회(1건조회) +
    @GetMapping("/galleryDb/edition/{uuid}")
    public String editGalleryDb(
            @PathVariable String uuid,
            Model model
    )

    {
//        1) 상세조회
        Optional<GalleryDb> optionalGalleryDb
                = galleryDbService.findById(uuid);
//        2) 결과 jsp 전달
        model.addAttribute("galleryDb", optionalGalleryDb.get());
        return "advanced/galleryDb/update_galleryDb.jsp";
    }

    //    2) 수정버튼 클릭시 update 함수
//    update -> put 방식 -> @PutMapping
    @PutMapping("/galleryDb/edit/{uuid}")
    public RedirectView updateGalleryDb(
            @PathVariable String uuid,
            @RequestParam (defaultValue = "") String galleryTitle,
            @RequestParam MultipartFile image
    )     {
        try {
//            DB 수정 : save()
            galleryDbService.save(uuid,galleryTitle,image);
        } catch (Exception e){
            log.debug(e.getMessage());
        }
//        강제 이동
        return new RedirectView("/advanced/galleryDb");
    }
}
