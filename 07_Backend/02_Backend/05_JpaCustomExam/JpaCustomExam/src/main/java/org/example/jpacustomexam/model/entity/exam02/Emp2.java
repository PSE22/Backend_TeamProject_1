 ...
 *                - @ManyToOne(fetch = FetchType.LAZY) : 의미 - @자식클래스(Many) To 부모클래스(One)
 *                  a. 조인 부모 클래스 속성(필드) 위에 붙임
 *                  b. 조인 관계 설정 어노테이션 : 1 : N 관계임
 *                  c. 옵션 : 실무에서는 무조건 LAZY 사용
 *                    가. fetch = FetchType.LAZY  : 나중에, 관계설정이 되더라도 그 엔티티가 필요할때 가져옴(성능향상)
 *                    나. fetch = FetchType.EAGER : 즉시,  관계설정된 엔티티 클래스 모두 가져옴
 *                      - N + 1 문제 발생 : 주의 해결방법이 있음 (양방향시 설명)
 *                       N + 1 문제 : A 테이블, B 테이블
 *                        A 테이블이 각 건수만큼 B 테이블 조회 select 문이 생성되는 현상
 ...
 *                  순환참조 : 조인 엔티티끼리 계속 무한히 조회를 반복하는것
 *                           (현상 : 무한 반복문과 비슷)
 *
 *
 *              (2) 양방향 조인 : 특수한 경우에 추가해서 사용, 잘못사용하면 성능 저하 있음
 *
 * <p>
 * ===========================================================
 * DATE            AUTHOR             NOTE
