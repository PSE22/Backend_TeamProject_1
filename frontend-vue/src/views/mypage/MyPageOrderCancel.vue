...
export default {
  ...
  methods: {
    async retriveOrder(orderId) {
      // 주문 상세 정보 조회
      try {
        let response = await MyOrderCheckService.get(orderId);
        this.opIds = response.data.map(detail => detail.opId); // opId 목록 추출
      } catch (e) {
    ...
    async confirmCancel() {
      try {
        let data = {
          ...
          opIds: this.opIds, // 전체 opId 목록
          ocPrice: this.ocPrice, // 취소 금액
